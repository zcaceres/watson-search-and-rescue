...
namespace Suimono.Core
{
	...
	public class SuimonoModule : MonoBehaviour {
		...
		public int layerWaterNum = 28;
		public string layerDepth;
		public int layerDepthNum = 29;
		public string layerScreenFX;
		public int layerScreenFXNum = 30;
		...
		void Start () {
			...
			underwaterDebrisTransform = underwaterDebris.GetComponent<Transform>();
			...
			if (sndparentobj != null){
				...
				for (int sx = 0; sx < (maxSounds); sx++){
					...
					soundObjectPrefab[sx].transform.parent = sndparentobj.transform;
		...
		void LateUpdate (){
			...
			if (transCamObject != null){
				...
				transCamObject.farClipPlane = transRenderDistance*1.2f;
			...
			if (causticCamObject != null){
				if (enableCaustics == false){
					...
					causticCamObject.farClipPlane = transRenderDistance*1.2f;
				}
		...
		public void AddFX(int fxSystem, Vector3 effectPos, int addRate, float addSize, float addRot, float addARot, Vector3 addVeloc, Color addCol){
			if (fxObject != null){
				...
				if (fxObject.fxParticles[fx] != null){
					...
					//set particles
					if (fxObject.fxParticles[fx].particleCount > 0.0f){
		...
		public void AddSoundFX(AudioClip sndClip, Vector3 soundPos, Vector3 sndVelocity){
			...
			if (enableInteraction){
				...
				if (playSounds && sndparentobj.defaultSplashSound.Length >= 1 ){
					setstep = sndparentobj.defaultSplashSound[modRand.Next(0,defaultSplashSound.Length-1)];
		...
		if (enableInteraction){
			...
			if (playSounds && sndparentobj.defaultSplashSound.Length >= 1 ){
				setstep = sndparentobj.defaultSplashSound[modRand.Next(0,sndparentobj.defaultSplashSound.Length-1)];
				setpitch = sndVelocity.y;
				setvolume = sndVelocity.z;
		...
		void checkUnderwaterEffects(){
			...
			if (Application.isPlaying){
				if (currentTransitionDepth > 0.1f){
		...
		if (Application.isPlaying){
			...
			//SET COLORS
			if (currentTransitionDepth > 0.1 && currentObjectIsOver==1.0f){
			    ...
			    if (suimonoObject != null && setCamera != null){
					...
					if (enableUnderwaterFX && suimonoObject.enableUnderwater && objectEnableUnderwaterFX==1.0f){
						...
						if (suimonoObject.enableUnderDebris){
				       		underwaterDebrisTransform.position = setCamera.transform.position;
						    underwaterDebrisTransform.rotation = setCamera.transform.rotation;
						    underwaterDebrisTransform.Translate(Vector3.forward * 5.0f);
						...
						if (underwaterObject != null){
							...
							if (causticObject != null){
								...
								if (Application.isPlaying){
									...
									if (causticObject != null){
										causticObject.heightFac = underwaterObject.hFac * 2.0f;
									}
								}
							}
						}
